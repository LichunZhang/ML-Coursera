...
h = sigmoid(a2 * Theta2');
y_out = ones(m, 1) * [1 : num_labels];
y_out = (y_out == repmat(y,1,num_labels));
% for c = 1 : num_labels
% 	J = J - sum(y_out(:, c) .* log(h(:, c)) + (1 - y_out(:, c)) .* log(1 - h(:, c)));
% end
J = J - sum(sum(y_out .* log(h) + (1 - y_out) .* log(1 - h)));
J = J / m;
J = J + 0.5 * lambda / m * (sum(sumsq(Theta1(:, 2:end))) + sum(sumsq(Theta2(:, 2:end))));
